...
/**
 * Base static class for performing query and update operations on the 'tblRoom' table.
 *
 * 
 *
...
abstract class BaseRoomPeer {
	...
	const NAME = 'tblRoom.NAME';

	/** the column name for the DESCRIPTION field */
	const DESCRIPTION = 'tblRoom.DESCRIPTION';
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseRoomPeer', $criteria, $con);
		}
	...
	/**
	 * Method to invalidate the instance pool of all tables related to tblRoom
	...
	{
		return $withPrefix ? RoomPeer::CLASS_DEFAULT : RoomPeer::OM_CLASS;
	}
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseRoomPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseRoomPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseRoomPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseRoomPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseRoomPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseRoomPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseRoomPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseRoomPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the tblRoom table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseRoomPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
